...
- Shared metadata envelope for CSV, XLS, and other readers while their native collectors are developed.
- Regression coverage for metadata round trips, encrypted updates, unknown-part preservation, arbitrary namespace prefixes, and CSV fallback metadata.
...
- Native ECMA-376 Agile encryption using AES-256-CBC, SHA-512 password derivation, verifier checks, and encrypted-package HMAC integrity.
- Native ECMA-376 Standard encryption using AES-128-ECB and SHA-1 for broad office-suite compatibility.
- Password-based decryption for Agile and Standard encrypted XLSX containers, including files produced by external office suites.
- Interoperable OLE/CFB v3 writer with mini streams, FAT, miniFAT, and DIFAT support for large encrypted packages.
- `ReaderOptions::withPassword()`, normal-session encrypted reads, and large-session encrypted streaming.
- `Xlsx::encryptFile()`, `decryptFile()`, `isEncrypted()`, `encryptionMode()`, and protection metadata APIs.
- `MnbExcel::encryptXlsx()`, `decryptXlsx()`, `isEncryptedXlsx()`, `xlsxEncryptionMode()`, and `xlsxProtection()` facade methods.
- Modern SHA-512 workbook and worksheet protection credentials, legacy verifier compatibility, configurable restrictions, and redacted protection inspection.
- Password-aware upload safety checks and XLSX integrity validation that inspect the decrypted inner package without retaining plaintext files.
- End-to-end encryption, wrong-password, upload, integrity, large-read, workbook-protection, and worksheet-protection regression coverage.

### Changed

- Version increased to `1.7.0`.
- The XLSX split package now requires `ext-openssl` and `ext-iconv` for standards-compatible Office encryption.
- The core, XLSX, application, and all split repositories require matching v1.7 releases; unchanged format/database packages remain compatible at their existing versions.
- Encrypted output uses a CFB v3 container for Microsoft Excel and LibreOffice interoperability while supporting DIFAT sectors for large files.

### Compatibility and security

- Agile AES-256 is the secure default; use `mode => standard` or `compatibility_mode => true` for older viewers.
- Both Agile and Standard files generated by the library were opened with the correct password in LibreOffice; Standard files generated by LibreOffice were decrypted by the library.
- Passwords are limited to 255 Unicode code points, matching the Office cryptography specification.
- Temporary decrypted packages are removed in `finally` blocks, and upload/integrity errors do not expose verifier or password material.
- Workbook/worksheet protection discourages editing but is not cryptographic confidentiality; use file encryption when secrecy is required.
