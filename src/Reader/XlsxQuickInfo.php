...
final class XlsxQuickInfo
{
    ...
    {
        $sheets = $this->sheetsInfo($path, array_replace($options, ['include_hidden' => true]));
        return $this->selectSheet($sheets, $sheet);
    }
    ...
    private function inspectSheet(string $realPath, ZipArchive $zip, array $sheet, bool $accurate): array
    {
        ...
        $lastColumnIndex = max((int) $dimension['columns'], (int) ($scan['last_column_index'] ?? 0));
        $lastRow = max((int) $dimension['rows'], (int) ($scan['last_row'] ?? 0));
